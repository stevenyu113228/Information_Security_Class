...
1. 透過讀取system的arg，判斷需要執行的加密算法
2. 如果傳入參數不為4，或加密的方式不屬於cipher變數內列舉的則視為輸入錯誤，回傳參數錯誤後結束程式
3. 如果參數正確，則將正確的值傳入指定的加密函數內，傳入參數為Cipher與Plaintext
4. 輸出加密函數的回傳密文
...
```
1. 將輸入的plaintext轉為ascii code並加上key的偏移量
2. 並且過濾出加過頭的數字，使之往回扣26
3. 將list轉string並回傳
...
```
1. 將plaintext轉ascii並減掉A的Ascii 成為0\~25
2. 把key轉ascii並減掉A的Ascii 成為0\~25
3. 將兩者的值xor
4. 把計算結果加上A的Ascii並回傳
